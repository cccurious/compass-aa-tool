...
const VERIFIED_FULLWIDTH = new Set(
    Array.from(
        '⌒´' +
            ...
            '★☆♀♂♪♭♯♠♣♥' + // F-1
            '♦☀☁☂☃☎☜☞✓' + // F-2（✕ は実機で消えるため除外）
            // 2026-07-26 の追加候補ラウンド（バッチ＋3 文字プローブ・☯ のみ 56/64 で除外）
            '⇋⇌⇧⇩⇵☖☗☝☟♡♢♤♧♨♩♬♮♻⚠⚽⚾✂➡〠〓〖〗〘〙〽〰' +
...
export const UNSAFE_DISPLAY_CHARS = new Set(
    Array.from(
        ...
        '▀▐░▒▓▘▝▖▗▚▞▛▜▙▟' +
            ...
            '↕' +
            // ☯ は iOS で 56/64（3 文字プローブ・先頭 ■ が 25px 手前）。
            // ブラウザ走査では全角に見える「↕ 型のすり抜け」の第 2 号
            '☯' +
            // 罫線: 破線
            ...
            // 2026-07-26 に 29 文字全数を個別プローブで実測完了。
            // ═╞╡╪（二重の横線＋単線の縦棒が右/左/貫通）だけが全角で復帰、
            // 残り 25 文字はすべて iOS 46/64（■ 間隔 179px）で端末差が確定
            '║╔╗╚╝╠╣╦╩╬╒╓╕╖╘╙╛╜╟╢╤╥╧╨╫' +
            // 罫線: 斜めと端点（╻╼╽╾╿ は同系統からの推定）
            '╴╵╶╷╸╹╺╻╼╽╾╿' + // ╱╲╳ は救済（同じ行の端点が不可視で巻き添え）
